...
class CPU {
  constructor(log_enabled = true) {
    ...
    this.logger =
      MODE == "browser" ? new logging.WebLogger() : new logging.Logger();
  ...
  debug_flags() {
    return (
      "[A: " +
      this.registers.A.toString(16) +
      " X: " +
      this.registers.X.toString(16) +
      " Y: " +
      this.registers.Y.toString(16) +
      " P: " +
      this.status_byte().toString(16) +
      " SP: " +
      this.registers.SP.toString(16) +
      " CYC: " +
      this.cycles % 400 +
      "]"
    );
  ...
  execute(instr_count = null) {
    ...
    while (this.running == true) {
      ...
      this.update_flags(
        optable[opcode].flags || [],
        optable[opcode].operand || ""
      );

      this.cycles += optable[opcode].cycles + optable[opcode].bytes;

      if (
        !optable[opcode].actions ||
        !optable[opcode].actions.includes("NO_UPDATE_PC")
      ) {
